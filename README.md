...
A Go binary with build info stripped (no module path or dependency
metadata recoverable, consistent with build info stripped or removed,
e.g. by an obfuscator like Gobfuscator) is likewise named in the
reasoning list without raising risk, routine build-time stripping is too
common in legitimate Go software to be distinguished from deliberate
obfuscation by this signal alone.
